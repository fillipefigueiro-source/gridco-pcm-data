...
class Gerador:
    ...
    def gerar(self, p, ligados):
        ...
        for no in self.nos:
            ...
            for ordem_pai, (pai_code, pai_idx) in enumerate(pais, 1):
                ...
                for j in self._indices(qtd, no, blocos, p.get("trackers_lista")):
    ...
    @staticmethod
    def _indices(qtd, no, blocos, lista=""):
        """Sequência de índices de um nó.

        Trackers: se a lista do as-built for colada ('1.101,2.101,...'), usa ela verbatim;
        senão gera o produto {linha}×{bloco}.
        """
        if no.get("bloco"):
            itens = [x.strip() for x in str(lista or "").replace(";", ",").split(",") if x.strip()]
            if itens:
                return itens
            return [f"{j}.{b}" for b in blocos for j in range(1, qtd + 1)]
        return list(range(1, qtd + 1))

    @staticmethod
    def _qtd(q, p, ordem_pai=1):
        """Quantidade fixa, parâmetro da tela, ou lista por pai ('4,2,4' = 4 no 1º QGBT, 2 no 2º...)."""
        if isinstance(q, int):
            return max(0, q)
        ...
        try:
            ...
            def processar_um(a):
                ...
                corpo = {"id_type_item": 1 if a["tipo"] == "L" else 2,
                         ...
                         "id_priority": a["prio"], "active": True, "visible_to_all": True,
