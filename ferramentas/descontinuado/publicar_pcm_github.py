# -*- coding: utf-8 -*-
"""
publicar_pcm_github.py
----------------------
Publica banco_dados.json no GitHub via API REST (substitui Power Automate).

Lê token do .env: GITHUB_TOKEN=ghp_xxxxx

Uso:
   py -3 publicar_pcm_github.py
"""
...
def publicar(token: str, payload_json: str, nrows: int) -> None:
    ...
    put_body = {
        "message": f"chore: refresh banco_dados.json [{nrows} OSs]",
        "content": content_b64,
...
def main() -> int:
    ...
    _log(f"Pasta: {pasta}")
    src = pasta / "banco_dados.json"
    if not src.exists():
        _log(f"ERRO: {src.name} não existe. Rode gerar_pcm_json.py primeiro.")
        return 2
    ...
    try:
        ...
        _log(f"ERRO: banco_dados.json corrompido: {e}")
        return 2

    nrows = 0
    if data.get("semanas"):
        nrows = sum(len(s.get("rows", [])) for s in data["semanas"])
    ...
    try:
        publicar(token, raw, nrows)
    except Exception as e:
        _log(f"ERRO ao publicar: {type(e).__name__}: {e}")
        return 4

    _log(f"OK: {nrows} OSs publicadas em {GITHUB_PATH}")
    return 0
