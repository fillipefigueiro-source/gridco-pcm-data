# -*- coding: utf-8 -*-
"""
publicar_supervisores_github.py
-------------------------------
Lê supervisores.json e publica em
   https://github.com/fillipefigueiro-source/gridco-pcm-data/supervisores.json
via API REST do GitHub (PUT contents).

Lê o token do .env: GITHUB_TOKEN=ghp_xxxxx

Uso:
   python publicar_supervisores_github.py
"""
...
def publicar(token: str, conteudo: bytes, tam: int) -> None:
    ...
    try:
        ...
        if e.code == 404:
            _log("  -> Arquivo ainda nao existe no repo, sera criado.")
        else:
            raise

    content_b64 = base64.b64encode(conteudo).decode("ascii")
    put_body = {
        "message": f"chore: refresh supervisores.json [{tam} bytes]",
        "content": content_b64,
...
def main() -> int:
    ...
    _log(f"Pasta: {pasta}")
    sup_path = pasta / "supervisores.json"
    if not sup_path.exists():
        _log(f"ERRO: {sup_path} nao existe. Rode gerar_supervisores_json.py primeiro.")
        return 2

    conteudo = sup_path.read_bytes()
    tam = len(conteudo)
    _log(f"  -> Lido: {tam} bytes")

    env = _carregar_env(pasta)
    token = env.get("GITHUB_TOKEN") or os.getenv("GITHUB_TOKEN")
    if not token:
        _log("ERRO: GITHUB_TOKEN nao definido no .env nem no ambiente.")
        return 3

    try:
        publicar(token, conteudo, tam)
    except Exception as e:
        _log(f"ERRO ao publicar: {type(e).__name__}: {e}")
        return 4

    _log(f"OK: supervisores.json publicado ({tam} bytes)")
    return 0
